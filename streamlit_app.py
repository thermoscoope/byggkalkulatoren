...
with tabs[9]:
    ...
    method = st.selectbox(
        ...
        format_func=lambda v: {
            "tan": tt("tan⁻¹(A / B)", "atan(A / B)"),
            "sin": tt("sin⁻¹(A / C)", "asin(A / C)"),
            "cos": tt("cos⁻¹(B / C)", "acos(B / C)"),
        }[v],
        index=0,
        key="ang_method",
    )
    ...
    unit = st.selectbox(
        tt("Enhet for inndata (samme enhet på alle sider)", "Input unit (use the same unit for all sides)"),
        ["mm", "cm", "m"],
        index=2,
        key="ang_unit",
    )
    ...
    st.caption(
        tt(
            "Bruk A, B og C som i Pytagoras: A og B er kateter, C er den lengste siden.",
            "Use A, B and C like in Pythagoras: A and B are the legs, C is the longest side.",
        )
    )
    ...
    with c3:
        C = st.number_input(
            tt("C (lengste side – trengs for sin/cos)", "C (longest side – needed for sin/cos)"),
            min_value=0.0,
            value=0.0,
            step=0.1,
            key="ang_C",
    ...
    A_mm = to_mm(float(A), unit)
    B_mm = to_mm(float(B), unit)
    C_mm = to_mm(float(C), unit) if float(C) > 0 else 0.0
    ...
    try:
        if method == "tan":
            if B_mm <= 0:
                err = tt("B må være > 0 for tan-formelen.", "B must be > 0 for the tan formula.")
            else:
                angle_deg = math.degrees(math.atan(A_mm / B_mm))

        elif method == "sin":
            if C_mm <= 0:
                err = tt("Du må fylle inn C (> 0) for sin-formelen.", "You must enter C (> 0) for the sin formula.")
            elif A_mm > C_mm:
                err = tt("A kan ikke være større enn C.", "A cannot be larger than C.")
            else:
                angle_deg = math.degrees(math.asin(A_mm / C_mm))

        else:  # cos
            if C_mm <= 0:
                err = tt("Du må fylle inn C (> 0) for cos-formelen.", "You must enter C (> 0) for the cos formula.")
            elif B_mm > C_mm:
                err = tt("B kan ikke være større enn C.", "B cannot be larger than C.")
            else:
                angle_deg = math.degrees(math.acos(B_mm / C_mm))
    except Exception:
...
if err:
        ...
        st.write(tt(f"Den andre spisse vinkelen i trekanten: {angle2:.2f}°", f"The other acute angle: {angle2:.2f}°"))
            ...
            if method == "tan":
                st.markdown(r"**Formel:**  \( \theta = \arctan(\frac{A}{B}) \)")
                st.markdown(rf"**Satt inn:**  \( \theta = \arctan(\frac{{{float(A):.4g}}}{{{float(B):.4g}}}) \)")
            elif method == "sin":
                st.markdown(r"**Formel:**  \( \theta = \arcsin(\frac{A}{C}) \)")
                st.markdown(rf"**Satt inn:**  \( \theta = \arcsin(\frac{{{float(A):.4g}}}{{{float(C):.4g}}}) \)")
            else:
                st.markdown(r"**Formel:**  \( \theta = \arccos(\frac{B}{C}) \)")
                st.markdown(rf"**Satt inn:**  \( \theta = \arccos(\frac{{{float(B):.4g}}}{{{float(C):.4g}}}) \)")
